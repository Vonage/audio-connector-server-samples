...
Optional:
  API_URL                      (default https://api.opentok.com for OpenTok path)
...
async def connect_audio_connector(
    *,
    ...
    def _call_vonage_connect() -> Any:
        logger.info("Using Vonage Application auth")
...
app = FastAPI(title="Vonage AI Triage Nurse (Pipecat Voice)", lifespan=lifespan)
...
async def index():
    return {
        "service": "vonage-ai-triage-nurse",
        "status": "ok",
        "hint": "POST /connect to attach Vonage audio to /ws",
    }
...
async def connect(request: Request) -> JSONResponse:
    ...
    # Support VONAGE_API_KEY/SECRET and OPENTOK_API_KEY/SECRET interchangeably
    api_key = os.getenv("VONAGE_API_KEY") or os.getenv("OPENTOK_API_KEY")
    api_secret = os.getenv("VONAGE_API_SECRET") or os.getenv("OPENTOK_API_SECRET")
    ...
    if application_id and private_key and not application_id.startswith("YOUR_"):
        ...
        # OpenTok / Vonage API key+secret path
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "or VONAGE_API_KEY and VONAGE_API_SECRET"
            ),
        )
    ...
    if not ws_uri:
        raise HTTPException(status_code=500, detail="Missing WS_URI env var")
